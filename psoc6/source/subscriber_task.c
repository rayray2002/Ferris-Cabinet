...
#define MQTT_SUBSCRIBE_RETRY_INTERVAL_MS (2000)
...
cy_mqtt_subscribe_info_t subscribe_info = {
    .qos = (cy_mqtt_qos_t)MQTT_MESSAGES_QOS, .topic = "MQTT_SUB_TOPIC", .topic_len = 14};

/******************************************************************************
 * Function Prototypes
 *******************************************************************************/
static void subscribe_to_topic(const char *topic);
static void unsubscribe_from_topic(const char *topic);
void print_heap_usage(char *msg);

void parse_config(const char *data) {
    printf("parse_config\r\n");
    ...
    const cJSON *desk = NULL;
    int index = 0;
    cJSON_ArrayForEach(desk, root) {
        cJSON *name = cJSON_GetObjectItemCaseSensitive(desk, "name");
        cJSON *description = cJSON_GetObjectItemCaseSensitive(desk, "description");
        cJSON *icon = cJSON_GetObjectItemCaseSensitive(desk, "icon");

        if (cJSON_IsString(name) && (name->valuestring != NULL)) {
            printf("Name: %s\r\n", name->valuestring);
            // desks_config[index].name = name->valuestring;
            strcpy(desks_config[index].name, name->valuestring);
        }
        if (cJSON_IsString(description) && (description->valuestring != NULL)) {
            printf("Description: %s\r\n", description->valuestring);
            // desks_config[index].description = description->valuestring;
            strcpy(desks_config[index].description, description->valuestring);
        }
        if (cJSON_IsNumber(icon)) {
            printf("Icon: %d\r\n", icon->valueint);
            desks_config[index].icon = icon->valueint;
        }
        index++;
    }
    num_of_desks = index;
...
void subscriber_task(void *pvParameters) {
    ...
    // subscribe_to_topic("/client/action");
    subscribe_to_topic("/server/config");
    subscribe_to_topic("/server/alarm");
        ...
        if (pdTRUE == xQueueReceive(subscriber_task_q, &subscriber_q_data, portMAX_DELAY)) {
            switch (subscriber_q_data.cmd) {
                ...
                case UNSUBSCRIBE_FROM_TOPIC: {
                    unsubscribe_from_topic(subscriber_q_data.topic);
                    break;
                }

                case UPDATE_DEVICE_STATE: {
                    printf("[Subscriber] Update device state with topic: %s\r\n",
                           subscriber_q_data.topic);
                    if (strcmp(subscriber_q_data.topic, "/server/config") == 0) {
                        printf("[Subscriber] Update /server/config\r\n");
                        printf("Payload: %s\r\n", subscriber_q_data.data);
                        parse_config(subscriber_q_data.data);
                    } else if (strcmp(subscriber_q_data.topic, "/server/alarm") == 0) {
                        printf("[Subscriber] Update /server/alarm\r\n");
                        printf("Payload: %s\r\n", subscriber_q_data.data);

                        alarm_command_t alarm_command;
                        alarm_command.music = (uint8_t)atoi(subscriber_q_data.data);
                        xQueueSend(alarm_command_q, &alarm_command, 0);
                    }

                    free(subscriber_q_data.data);
                    free(subscriber_q_data.topic);

                    print_heap_usage("subscriber_task: After updating device state");
                    break;
                }
            }
        }
 ...
 ******************************************************************************/
static void subscribe_to_topic(const char *topic) {
    // printf("Subscribing to the topic '%s'...\n", topic);
    /* Status variable */
...
void mqtt_subscription_callback(cy_mqtt_publish_info_t *received_msg_info) {
    ...
    printf(
        ...
        "    Publish QoS: %d\n"
        "    Publish payload: %.*s\n\n",
        received_msg_info->topic_len, received_msg_info->topic, (int)received_msg_info->qos,
    ...
    char *topic = malloc(received_msg_info->topic_len + 1);
    memcpy(topic, received_msg_info->topic, received_msg_info->topic_len);
    topic[received_msg_info->topic_len] = '\0';
    subscriber_q_data.topic = topic;

    char *data = malloc(received_msg_info->payload_len + 1);
    memcpy(data, received_msg_info->payload, received_msg_info->payload_len);
    data[received_msg_info->payload_len] = '\0';
    subscriber_q_data.data = data;
...
static void unsubscribe_from_topic(const char *topic) {
    ...
    cy_rslt_t result = cy_mqtt_unsubscribe(
        mqtt_connection, (cy_mqtt_unsubscribe_info_t *)&subscribe_info, SUBSCRIPTION_COUNT);
