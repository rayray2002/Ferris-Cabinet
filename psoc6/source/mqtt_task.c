...
void mqtt_client_task(void *pvParameters) {
    ...
    /* Create the http task and cleanup if the operation fails. */
    if (pdPASS != xTaskCreate(http_client_task, "http task", HTTP_CLIENT_TASK_STACK_SIZE, NULL,
                              HTTP_CLIENT_TASK_PRIORITY, &http_client_task_handle)) {
        printf("Failed to create the http task!\n");
        goto exit_cleanup;
    }

    /* Create the subscriber task and cleanup if the operation fails. */
    if (pdPASS != xTaskCreate(subscriber_task, "Subscriber task", SUBSCRIBER_TASK_STACK_SIZE, NULL,
    ...
    if (pdPASS != xTaskCreate(publisher_task, "Publisher task", PUBLISHER_TASK_STACK_SIZE, NULL,
...
static cy_rslt_t wifi_connect(void) {
    ...
    if (cy_wcm_is_connected_to_ap() == 0) {
        ...
        for (uint32_t retry_count = 0; retry_count < MAX_WIFI_CONN_RETRIES; retry_count++) {
            ...
            printf(
                "Connection to Wi-Fi network failed with error code 0x%0X. Retrying in %d ms. "
                "Retries left: %d\n",
...
static cy_rslt_t mqtt_init(void) {
    ...
    result =
        cy_mqtt_create(mqtt_network_buffer, MQTT_NETWORK_BUFFER_SIZE, security_info, &broker_info,
...
static cy_rslt_t mqtt_connect(void) {
    ...
    printf("\nMQTT client '%.*s' connecting to MQTT broker '%.*s'...\n\n",
           connection_info.client_id_len, connection_info.client_id, broker_info.hostname_len,
           broker_info.hostname);

    for (uint32_t retry_count = 0; retry_count < MAX_MQTT_CONN_RETRIES; retry_count++) {
        if (cy_wcm_is_connected_to_ap() == 0) {
            printf(
                "Unexpectedly disconnected from Wi-Fi network! Initiating Wi-Fi reconnection...\n");
            status_flag &= ~(WIFI_CONNECTED);
        ...
        printf(
            "MQTT connection failed with error code 0x%0X. Retrying in %d ms. Retries left: %d\n",
...
static cy_rslt_t mqtt_get_unique_client_identifier(char *mqtt_client_identifier) {
    ...
    if (0 > snprintf(mqtt_client_identifier, (MQTT_CLIENT_IDENTIFIER_MAX_LEN + 1),
                     MQTT_CLIENT_IDENTIFIER "%lu", (long unsigned int)Clock_GetTimeMs())) {
