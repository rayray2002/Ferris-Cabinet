...
#define MQTT_CLIENT_TASK_PRIORITY (configMAX_PRIORITIES - 2)
#define TASK_INTERFACE_PRIORITY (configMAX_PRIORITIES - 3)
#define ALARM_TASK_PRIORITY (0)

/* Stack sizes of user tasks in this project */
#define TASK_CAPSENSE_STACK_SIZE (256u)
#define TASK_INTERFACE_STACK_SIZE (1024 * 1)
#define MQTT_CLIENT_TASK_STACK_SIZE (1024 * 1)
#define ALARM_TASK_STACK_SIZE (1024 * 1)
...
int main(void) {
    ...
    capsense_event_data_q = xQueueCreate(SINGLE_ELEMENT_QUEUE, sizeof(capsense_event_data_t));
    alarm_command_q = xQueueCreate(SINGLE_ELEMENT_QUEUE, sizeof(alarm_command_t));

    /* Create the user tasks. See the respective task definition for more
     * details of these tasks.
     */
    xTaskCreate(task_capsense, "CapSense Task", TASK_CAPSENSE_STACK_SIZE, NULL,
                TASK_CAPSENSE_PRIORITY, NULL);
    xTaskCreate(task_interface, "Interface Task", TASK_INTERFACE_STACK_SIZE, NULL,
                TASK_INTERFACE_PRIORITY, NULL);
    xTaskCreate(mqtt_client_task, "MQTT Client task", MQTT_CLIENT_TASK_STACK_SIZE, NULL,
                MQTT_CLIENT_TASK_PRIORITY, NULL);
    xTaskCreate(alarm_task, "Alarm Task", ALARM_TASK_STACK_SIZE, NULL, ALARM_TASK_PRIORITY, NULL);
