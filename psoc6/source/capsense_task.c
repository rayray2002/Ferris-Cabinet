...
#define EZI2C_INTERRUPT_PRIORITY                                            \
 ...
 * Global variables
 ******************************************************************************/
QueueHandle_t capsense_event_data_q;
...
void task_capsense(void* param) {
    ...
    scan_timer_handle = xTimerCreate("Scan Timer", CAPSENSE_SCAN_INTERVAL_MS, pdTRUE, NULL,
                                     capsense_timer_callback);
...
static void process_touch(void) {
    ...
    static uint8_t slider_touched_prev = 0;
    ...
    if ((0u != button0_status) && (0u == button0_status_prev)) {
        printf("[CapSense Task] Button0 pressed\r\n");
        capsense_event_data.event = CAPSENSE_BUTTON0_PRESSED;
        send_event = true;
    }

    /* Detect new touch on Button1 */
    if ((0u != button1_status) && (0u == button1_status_prev)) {
        printf("[CapSense Task] Button1 pressed\r\n");
        capsense_event_data.event = CAPSENSE_BUTTON1_PRESSED;
        send_event = true;
    }

    /* Detect new touch on slider */
    if ((0u != slider_touched) && (0u == slider_touched_prev)) {
        // record current slider position as the initial position
        // printf("[CapSense Task] Slider touched\r\n");
        slider_pos_prev = slider_pos;
    }
    /* Detect release on slider */
    if ((0u == slider_touched) && (0u != slider_touched_prev)) {
        // compare current slider position with the initial position
        // to determine swiping left or right
        // printf("[CapSense Task] Slider released\r\n");
        if (slider_pos > slider_pos_prev) {
            capsense_event_data.event = CAPSENSE_SLIDER_RIGHT;
            printf("[CapSense Task] Slider swiped right\r\n");
        } else if (slider_pos < slider_pos_prev) {
            capsense_event_data.event = CAPSENSE_SLIDER_LEFT;
            printf("[CapSense Task] Slider swiped left\r\n");
        }
        send_event = true;
    }
    ...
    // {
    //     double slider_percent = (double)slider_pos /
    //     cy_capsense_context.ptrWdConfig[CY_CAPSENSE_LINEARSLIDER0_WDGT_ID].xResolution;

    //     capsense_event_data.event = CAPSENSE_SLIDER_TOUCHED;
    //     capsense_event_data.percent = slider_percent;
    //     send_event = true;
    //     printf("[CapSense Task] Slider touched, slider_percent = %f\r\n", slider_percent);
    // }

    /* Send command to update LED state if required */
    if (send_event) {
        xQueueSendToBack(capsense_event_data_q, &capsense_event_data, 0u);
    }

    /* Update previous touch status */
    button0_status_prev = button0_status;
    button1_status_prev = button1_status;
    slider_touched_prev = slider_touched;
}
...
static uint32_t capsense_init(void) {
    ...
    cyhal_system_set_isr(csd_interrupt_IRQn, csd_interrupt_IRQn, CAPSENSE_INTERRUPT_PRIORITY,
                         &capsense_isr);
    ...
    status = Cy_CapSense_RegisterCallback(CY_CAPSENSE_END_OF_SCAN_E, capsense_end_of_scan_callback,
                                          &cy_capsense_context);
    if (CYRET_SUCCESS != status) {
        return status;
    }
 ...
 *
 *******************************************************************************/
static void capsense_isr(void) { Cy_CapSense_InterruptHandler(CYBSP_CSD_HW, &cy_capsense_context); }
